...
def plot_eigenvalues(L0, L_sketched, L_randomized, real_eigens, ylog = False, show_sub = False):
    """
...
def plot_ritz_sweep(
    ...
    mat = PolyDecayMatrix(n=p, R=10, d=d)
    ...
    for k in k_list:
        for s in s_list:
            ...
            L0, Ls, Lr, eigs_k = collect_ritz_values(p=p, k=k, G_matvec=G_matvec, real_eigens=true_eigs, sketch=sketch, verbose=verbose)
            # L0, Ls, Lr, eigs_k = collect_ritz_values(p, k, G_matvec, true_eigs, sketch, verbose=False)
            plot_eigenvalues(L0, Ls, Lr, eigs_k, ylog=False)
