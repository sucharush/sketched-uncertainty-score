...
from synthetic_utils.norm_collect import run_experiment_grid


def make_type_map(k_fixed, s_fixed):
    return {
        "sweep_s": (f"(1) Varying $s$, fixed $k={k_fixed}$", "$s$"),
        "fixed_s": (f"(2) Fixed $s={s_fixed}$, varying $k$", "$k$"),
        "s_eq_2k": ("(3) $s=2k$, varying $k$", "$k$")
    }
...
def build_comparison_data(cfg, sketch_class):
    ...
    base_df = all_df[all_df["Method"] == "High Memory"]
    diff_df = all_df[all_df["Method"] != "High Memory"].copy()
...
def plot_comparison_grid(cfg, all_df, merged_df, **kwargs):
    ...
    for i, t in enumerate(["sweep_s", "fixed_s", "s_eq_2k"]):
        sns.pointplot(
            ...
            x="xval", y="Value", hue="Method",
            dodge=0.1, errorbar="sd", join=True, markers="o", linestyles="-",
            linewidth=linewidth, capsize=0.1, palette=palette, ax=axes[0, i]
        )
        ...
        sns.pointplot(
            ...
            x="xval", y="AbsDiff", hue="Method",
            dodge=0.1, errorbar="sd", join=True, markers="o", linestyles="-",
            linewidth=linewidth, capsize=0.1, palette=palette, ax=axes[1, i]
        )
